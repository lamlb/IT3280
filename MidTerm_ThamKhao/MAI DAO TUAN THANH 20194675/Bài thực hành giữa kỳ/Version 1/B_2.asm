#B2.Nhập mảng số nguyên từ bàn phím. Sắp xếp các phần tử có giá trị dương giảm dần
.data
	s1:	.asciiz	"Nhap so phan tu cua mang: "
...
.text
	...
	mul 	$s1, $s0, 4 		# s1 = 4 * N
	sub 	$sp, $sp, $s1		# Khoi tao bo nho stack 4*n byte de lưu cac phan tu cua mang
	la	$a0, s2
	li	$v0, 4
	syscall
loop_scan:
...
continue_i:
	...
	j	loop_i	
end_loop_i:
	lw 	$s0, 0($sp)		# day nguoc lai tu stack ra ngoai
	lw 	$ra, 4($sp)
	jr 	$ra
end_sort:
exit:
